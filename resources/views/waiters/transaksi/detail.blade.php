        ...
        <div class="col-12">
            ...
            <div class="white-box">
                ...
                <table class="table table-bordered">
                    ...
                    <tr>
                        <th>Bagian</th>
                    @if (!empty($transaksi->meja) && $transaksi->bagian_id != 18)
                    ...
                    <tr>
                        <th>Catatan</th>
                        <td>
                            <form action="{{ route('waiters.transaksi.updateCatatan', $transaksi->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                ...
                {{-- FORM TAMBAH --}}
                <form class="row g-2 mb-3" method="POST" action="{{ route('waiters.transaksi.detail.store') }}">
                    @csrf
                    ...
                    <tbody>
                        @foreach ($transaksi->transaksidetail as $d)
                            <tr>
                                ...
                                <td>
                                    <form method="POST" action="{{ route('waiters.transaksi.detail.update', $d->id) }}"
                                        class="d-flex">
                                        @csrf
                                        @method('PUT')
                                ...
                                <td>
                                    <form action="{{ route('waiters.transaksi.detail.delete', $d->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
